...
mod tests {
    ...
    fn can_generate_xcoords() {
        let n = 10;
        let x = crate::symmetric_airfoil::generate_symmetric_xcoords(1.0, n);
...
pub mod symmetric_airfoil {
    ...
    pub fn slope_of_camber_line(m: f64, p: f64, x_div_c: f64, c: f64) -> f64 {
        if x_div_c <= p {
    ...
    pub fn camber_line(m: f64, p: f64, x_div_c: f64, c: f64) -> f64 {
        if x_div_c <= p {
    ...
    pub fn generate_symmetric_airfoil(_m: f64, _p: f64, t: f64, chord_length: f64, num_samples: i32) -> (Vec<f64>, Vec<f64>) {
    ...
    pub fn symmetric_0020_airfoil_ordinate(x: f64, chord_length: f64) -> f64 {
        chord_length * (
            (A0 * (x / chord_length).sqrt()) + 
            (A1 * (x / chord_length)) + 
            (A2 * (x / chord_length).powf(2.0)) +
            (A3 * (x / chord_length).powf(3.0)) + 
            (A4 * (x / chord_length).powf(4.0)))
    }
}
